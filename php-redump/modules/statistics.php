...
<tr><th></th><th>World</th><th>Europe</th><th>USA</th><th>Asia</th><th>'.status(4).'</th><th>'.status(5).'</th><th>Total</th></tr>';

$total_discs = 0;

while ($system = $systems->fetch_array()) {
	$europe = $mysqli->query('SELECT COUNT(`d`.`d_id`) FROM `discs` AS `d`,`systems` AS `s` WHERE `d`.`d_media`=`s`.`s_id` AND `s`.`s_short`="'.$system['s_short'].'" AND `d`.`d_status`<>1 AND `d`.`d_status`<>2 AND `d`.`d_region`<>"U" AND `d`.`d_region`<>"J" AND `d`.`d_region`<>"A" AND `d`.`d_region`<>"K" AND `d`.`d_region`<>"W"')->fetch_array();
	...
	$asia   = $mysqli->query('SELECT COUNT(`d`.`d_id`) FROM `discs` AS `d`,`systems` AS `s` WHERE `d`.`d_media`=`s`.`s_id` AND `s`.`s_short`="'.$system['s_short'].'" AND `d`.`d_status`<>1 AND `d`.`d_status`<>2 AND (`d`.`d_region`="J" OR `d`.`d_region`="A" OR `d`.`d_region`="K")')->fetch_array();
	$world  = $mysqli->query('SELECT COUNT(`d`.`d_id`) FROM `discs` AS `d`,`systems` AS `s` WHERE `d`.`d_media`=`s`.`s_id` AND `s`.`s_short`="'.$system['s_short'].'" AND `d`.`d_status`<>1 AND `d`.`d_status`<>2 AND `d`.`d_region`="W"')->fetch_array();
