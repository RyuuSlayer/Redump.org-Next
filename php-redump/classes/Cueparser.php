	...
	function loadCuesheet($cue) {
		...
		foreach ($cue as $v) {
			...
			if (preg_match('@^(FILE "[^"]+" (BINARY|WAVE))?$@', $v)) {
				...
				$file_opened = false;
				$track_opened = false;
			} else if (preg_match('@^INDEX 01 ([0-9]{2}):([0-9]{2}):([0-9]{2})$@', $v, $matches)) {
				if (!$file_opened) {
					return CUEPARSER_ERROR_FILENOTOPENED;
				}
				if (!$track_opened) {
				...
				$this->tracks[$this->trackscount]['pregap'] = $min * 75 * 60 + $sec * 75 + $fr;
	...
	function returnCuesheet() {
		...
		foreach ($this->tracks as $track) {
			...
			if (!$track['pregap']) {
				...
				$gapmin = str_pad(floor($track['pregap'] / 60 / 75), 2, '0', STR_PAD_LEFT);
				$gapsec = str_pad(floor($track['pregap'] / 75) % 60, 2, '0', STR_PAD_LEFT);
				$gapframe = str_pad($track['pregap'] % 75, 2, '0', STR_PAD_LEFT);
				$cue .= "    INDEX 01 ".$gapmin.':'.$gapsec.':'.$gapframe."\r\n";			
			}
		}
		return $cue;
	}
